...
class ValueIterationTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        iterations = int(testDict['value_iterations'])
        if 'noise' in testDict: self.grid.set_noise(float(testDict['noise']))
        if 'living_reward' in testDict: self.grid.set_living_reward(float(testDict['living_reward']))
        maxPreIterations = 10
    ...
    def runAgent(self, moduleDict, numIterations):
        ...
        for state in states:
            ...
            possibleActions = self.grid.get_possible_actions(state)
            for action in actions:
                if action not in qValues:
                    qValues[action] = {}
                if action in possibleActions:
                    qValues[action][state] = agent.compute_q_value_from_values(state, action)
                else:
...
class ApproximateQLearningTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        if 'living_reward' in testDict: self.grid.set_living_reward(float(testDict['living_reward']))
        self.grid = gridworld.Gridworld(parseGrid(testDict['grid']))
        self.env = gridworld.Gridworld_environment(self.grid)
        ...
        num_experiences = int(testDict['num_experiences'])
        maxPreExperiences = 10
        self.numsExperiencesForDisplay = list(range(min(num_experiences, maxPreExperiences)))
        self.testOutFile = testDict['test_out_file']
        ...
        if maxPreExperiences < num_experiences:
            self.numsExperiencesForDisplay.append(num_experiences)
    ...
    def executeNExperiences(self, grades, moduleDict, solutionDict, n):
        ...
        if lastExperience is not None:
            fileOutString += "Agent observed the transition (start_state = %s, action = %s, endState = %s, reward = %f)\n\n" % lastExperience
    ...
    def runAgent(self, moduleDict, num_experiences):
        ...
        for state in states:
            ...
            for action in actions:
                if action not in qValues:
                    qValues[action] = {}
                if action in possibleActions:
                    qValues[action][state] = agent.get_q_value(state, action)
                else:
                    qValues[action][state] = None
        qValuesPretty = {}
        for action in actions:
...
class EpsilonGreedyTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.epsilon = float(testDict['epsilon'])
        self.learning_rate = float(testDict['learning_rate'])
        self.num_experiences = int(testDict['num_experiences'])
    ...
    def testEpsilonGreedy(self, moduleDict, tolerance=0.025):
        ...
        for state in self.grid.get_states():
            numLegalActions = len(agent.get_legal_actions(state))
            if numLegalActions <= 1:
                continue
            numGreedyChoices = 0
            optimalAction = agent.compute_action_from_q_values(state)
            for iteration in range(self.numIterations):
                ...
                if agent.get_action(state) == optimalAction:
                    numGreedyChoices += 1
...
def computePolicy(moduleDict, grid, discount):
    ...
    for state in grid.get_states():
        policy[state] = valueIterator.compute_action_from_values(state)
    return policy
...
class GridPolicyTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.grid = gridworld.Gridworld(parseGrid(testDict['grid']))
        self.grid_name = testDict['grid_name']
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        ## check path
        path = followPath(policy, self.grid.get_start_state())
