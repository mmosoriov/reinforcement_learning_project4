...
#        self.setupSimulationButtons(win)
    ...
    def __init__(self, win):
        ...
        action_fn = lambda state: \
          self.robotEnvironment.get_possible_actions(state)
        self.learner = qlearningAgents.QLearningAgent(action_fn=action_fn)
    ...
    def step(self):
        ...
        state = self.robotEnvironment.get_current_state()
        actions = self.robotEnvironment.get_possible_actions(state)
        if len(actions) == 0.0:
        ...
        next_state, reward = self.robotEnvironment.do_action(action)
            ...
            for j in range(100):
                ...
                if not self.learner.seenState(state):
                    ...
                    for action in ('kickLeft','kickRight','doNothing'):
                        qVal = self.learner.get_q_value(state, action)
