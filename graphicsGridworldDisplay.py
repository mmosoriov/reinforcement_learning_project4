...
class GraphicsGridworldDisplay:
    ...
    def display_values(self, agent, currentState = None, message = 'Agent Values'):
        values = util.Counter()
        policy = {}
    ...
    def display_null_values(self, currentState = None, message = ''):
        values = util.Counter()
        #policy = {}
        states = self.gridworld.get_states()
        for state in states:
    ...
    def display_q_values(self, agent, currentState = None, message = 'Agent Q-Values'):
...
def drawValues(gridworld, values, policy, currentState = None, message = 'State Values'):
    grid = gridworld.grid
    blank()
    valueList = [values[state] for state in gridworld.get_states()] + [0.0]
...
def drawQValues(gridworld, qValues, currentState = None, message = 'State-Action Q-Values'):
    grid = gridworld.grid
    blank()
    stateCrossActions = [[(state, action) for action in gridworld.get_possible_actions(state)] for state in gridworld.get_states()]
...
def drawNullSquare(grid,x, y, isObstacle, is_terminal, isCurrent):
    ...
    if is_terminal and not isObstacle:
        square( (screen_x, screen_y),
                     0.4* GRID_SIZE,
                     color = EDGE_COLOR,
                     filled = 0,
...
def drawSquare(x, y, val, min, max, valStr, action, isObstacle, is_terminal, isCurrent):
