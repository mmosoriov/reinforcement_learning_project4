...
class TextGridworldDisplay:
    ...
    def display_values(self, agent, currentState = None, message = None):
        if message != None:
            print(message)
        values = util.Counter()
        policy = {}
    ...
    def display_null_values(self, agent, currentState = None, message = None):
        if message != None: print(message)
        prettyPrintNullValues(self.gridworld, currentState)

    def display_q_values(self, agent, currentState = None, message = None):
        if message != None: print(message)
...
def prettyPrintValues(grid_world, values, policy=None, currentState = None):
...
def prettyPrintNullValues(grid_world, currentState = None):
    grid = grid_world.grid
    maxLen = 11
    newRows = []
    for y in range(grid.height):
...
def prettyPrintQValues(grid_world, qValues, currentState=None):
    grid = grid_world.grid
    maxLen = 11
    newRows = []
    for y in range(grid.height):
        ...
        for x in range(grid.width):
            ...
            if state == currentState and state == grid_world.get_start_state():
                ewString = westString + " S:* " + eastString
...
if __name__ == '__main__':
    ...
    grid = gridworld.get_cliff_grid3()
