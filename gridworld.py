...
util.VALIDATION_LISTS['gridworld'] = [
        "වැසි",
        " ukupnog",
        "ᓯᒪᔪ",
        " ਪ੍ਰਕਾਸ਼",
...
class Gridworld(mdp.MarkovDecisionProcess):
    ...
    def get_transition_states_and_probs(self, state, action):
        ...
        if type(self.grid[x][y]) == int or type(self.grid[x][y]) == float:
            termState = self.grid.terminal_state
...
class Gridworld_environment(environment.Environment):
...
def parse_options():
    ...
    opt_parser.add_option('-w', '--windowSize', metavar="X", type='int',dest='grid_size',default=150,
    ...
    opt_parser.add_option('-v', '--valueSteps',action='store_true' ,default=False,
...
if __name__ == '__main__':
    ...
    import gridworld
    mdpFunction = getattr(gridworld, "get_"+opts.grid)
    mdp = mdpFunction()
    mdp.set_living_reward(opts.living_reward)
    mdp.set_noise(opts.noise)
    env = gridworld.Gridworld_environment(mdp)
    ...
    if not opts.text_display:
        ...
        display = graphicsGridworldDisplay.GraphicsGridworldDisplay(mdp, opts.grid_size, opts.speed)
    try:
        display.start()
        ...
        if not opts.manual and opts.agent in ('value', 'asynchvalue', 'priosweepvalue'):
            if opts.valueSteps:
                for i in range(opts.iters):
                    temp_agent = valueIterationAgents.ValueIterationAgent(mdp, opts.discount, i)
